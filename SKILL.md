...
2. 分析新需求（截图或文字描述）
3. 对比差异，分类变更，构造 changes.json
4. 【标准格式】直接调用 scripts/generate.py 生成 xlsx ← 唯一正确路径
   【非标准格式】临时生成脚本（见 Step 5）
```
...
**→ 必须执行 Step 3 → Step 4，禁止跳过，禁止自己写脚本。**

### ⚠️ 输出 CUSTOM → 走临时脚本路径

列名不匹配 → 根据实际列名临时生成 Python 脚本，参考 Step 5。
